...
with (obj_cursor) {
	...
	var hps = string( player_health ) + " Health";
	var hpw = string_width( string_hash_to_newline( hps ) );
	var hpx = floor( x - hpw / 2 );
	var hpy = floor( y - 12 - ( sh * 2 ) );
	draw_rect_ext( hpx, hpy + 1, hpw, sh - 2, c_black, 0.7 );
	draw_text( hpx + 1, hpy, string_hash_to_newline( hps ) );
}
